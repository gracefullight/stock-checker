...
export function calculateAllIndicators(data: {
  ...
  // Calculate SMA50
  const sma50 =
    closes.length >= 50 ? closes.slice(-50).reduce((sum, price) => sum + price, 0) / 50 : NaN;

  // Calculate SMA200
  const sma200 =
    closes.length >= 200 ? closes.slice(-200).reduce((sum, price) => sum + price, 0) / 200 : NaN;
...
export function calcRecentMacdHistogram(closes: number[], n = 5): number[] {
  const macdValues = MACD.calculate({
    values: closes,
    fastPeriod: 12,
    slowPeriod: 26,
  ...
  return macdValues.slice(-n).map((m) => {
