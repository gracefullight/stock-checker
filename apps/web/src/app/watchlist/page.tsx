...
import { getScreener, getWatchlist } from '@/lib/api';
...
export default async function WatchlistPage() {
  ...
  try {
    tickers = await getWatchlist();
  } catch (err) {
    error = err instanceof Error ? err.message : 'Failed to fetch watchlist';
  }
        ...
        <>
          <Card className="mb-4">
            <CardHeader className="border-b border-border py-1.5 px-3">
              <span className="text-[10px] font-mono text-muted-foreground">MANAGE WATCHLIST</span>
            </CardHeader>
            <CardContent className="pt-3">
              <WatchlistControls tickers={tickers} />
            </CardContent>
          </Card>
            ...
            <Card>
              <CardContent className="py-8 text-center">
                <p className="font-mono text-xs text-muted-foreground">
                  No watched tickers yet. Add a ticker to start tracking.
                </p>
              </CardContent>
            </Card>
          ) : (
            <WatchlistResults tickers={tickers} />
          )}
        </>
      )}
    </div>
...
async function WatchlistResults({ tickers }: { tickers: string[] }) {
